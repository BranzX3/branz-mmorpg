...
- `MMO_CLIENT_ACCEPTANCE_LEGACY_MAIN_HAND_MIGRATION_V1` — Section A only. The built-in action owns the exact legacy revision `8c5a04271f9385730aff0b3332608812a216dc95`, exact target revision `2bfbcc74f81a57fb6d4e61151efba4446eff75d5`, fixed worker-root worktree names, fixed client phases and the normal `mmo-bootstrap/run/plugins/BranzMMO/embedded-postgres` path. It seeds MAIN_HAND through the legacy `/mmo dev` + Chronicle transaction path, captures `/paper dumpitem`, switches the same action-owned runtime worktree to the exact target while preserving that database directory, checks UUID/version/durability/reconnect/restart evidence, and separately repeats the exact switch for a full-inventory fail-closed case. The task manifest cannot select either revision, command, argv, environment, regex, checkout path or database path.
