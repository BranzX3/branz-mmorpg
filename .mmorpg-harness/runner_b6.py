#!/usr/bin/env python3
"""B6 Chronicle-slot extension for the immutable MMORPG harness runner core."""
...
def evaluate_chronicle_slot_checks(client_text: str, paper_text: str) -> dict[str, bool]:
    slot_nine_cancelled = re.search(
        r"PHYSICAL_AUTHORITY_HOTBAR_CLICK_MONITOR_SERVER .*?slot=8 cancelled=true "
        r"sameInventory=true currentProjection=false .*?playerCursorProjection=true",
        paper_text,
    )
    return {
        ...
        "chronicle_slot9_mouse_client":
            "PHYSICAL_AUTHORITY_CHRONICLE_SLOT9_MOUSE_SENT_CLIENT" in client_text,
        "chronicle_rejection_message_client":
            "PHYSICAL_AUTHORITY_CHRONICLE_REJECTION_MESSAGE_CLIENT" in client_text,
        ...
        "chronicle_slot9_cancelled_server": slot_nine_cancelled is not None,
        "chronicle_no_hotbar_commit_server":
            "PHYSICAL_AUTHORITY_HOTBAR_MOVE_COMMITTED_SERVER" not in paper_text,
        "chronicle_no_sequence_invalid_server":
            "PHYSICAL_AUTHORITY_HOTBAR_SEQUENCE_INVALID_SERVER" not in paper_text,
        "chronicle_status_command_count_server": paper_text.count("/mmo physical status") == 3,
    }


def install(core: Any) -> None:
    """Install one fixed B6 capability while preserving the reviewed core implementation."""
    ...
    def action_client_acceptance_chronicle_slot(repo, result_dir, manifest):
        ...
        checks.update(evaluate_chronicle_slot_checks(client_text, paper_text))
    ...
    core.evaluate_chronicle_slot_checks = evaluate_chronicle_slot_checks
